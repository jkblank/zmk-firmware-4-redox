...
- `boards/shields/redox_handwire/` — the custom shield definition for this handwired Redox (source keyboard design: https://github.com/mattdibi/redox-keyboard):
  ...
  - `redox_handwire.dtsi` — the shared devicetree: GPIO matrix kscan (5 rows × 7 cols per half, col2row diode direction, pin assignment traced from the actual hand-wired board) and the `matrix_transform` mapping physical positions to a 14-column logical layout (left = cols 0–6, right = cols 7–13, right half auto-offset by the central's column count).
...
- `redox_handwire_left.overlay` (left/central only) also wires a dedicated physical unlock button on pro_micro pin 0 (SoC P0.08, pin-to-GND, internal pull-up) via `zmk,kscan-gpio-direct` + `zmk,kscan-sideband-behaviors`, firing `&studio_unlock` directly. This is independent of the `&lt 1 SPACE` / `studio_layer` hold-tap in the keymap - either one unlocks Studio.
